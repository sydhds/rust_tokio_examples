...
fn main() {
    ...
    let path_2 = PathBuf::from("image_gray.ppm");

    println!("Generating image data...");
    let mut img = generate_image_data(width, height);
    ...
    img2.chunks_exact_mut(3).for_each(|chunk| {
        let r = chunk[0];
        let g = chunk[1];
        let b = chunk[2];
    ...
    img.par_chunks_exact_mut(3).for_each(|chunk| {
        let r = chunk[0];
        let g = chunk[1];
        let b = chunk[2];
    ...
    println!("Writing {:?}...", path_2);
    write_image(width, height, &img, path_2);
}

fn generate_image_data(width: u16, height: u16) -> Vec<u8> {
    // let data_size = width * height * 3;
    let data_size = usize::try_from(u32::from(width) * u32::from(height) * 3).unwrap();
...
fn write_image(width: u16, height: u16, data: &Vec<u8>, path: PathBuf) {
    ...
    for j in 0..height {
        for i in 0..width {
            ...
            let index = usize::try_from(index_).unwrap();
