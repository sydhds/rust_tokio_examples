...
fn main() {
    ...
    if let Cow::Borrowed(_) = c2 {
        println!("c2 is still borrowed!");
    }
    ...
    if let Cow::Owned(_) = c2_1 {
        println!("c2 is now owned!");
    }

    // Cow in a struct
    let path = "bar.txt";
    let mycow = MyCow {
        c: replace_txt_ext_to_md_ext(path),
    };
    println!("mycow: {}", mycow.c);
    if let Cow::Owned(_) = mycow.c {
        println!("mycow.c is owned!");
    }
    ...
    println!("mycow: {}", mycow.c);
    if let Cow::Borrowed(_) = mycow.c {
        println!("mycow.c is borrowed!");
    }
    ...
    */
    ...
    if let Cow::Borrowed(_) = ic_1 {
        println!("ic_1 is borrowed, value: {}", ic_1);
    }
    let ic_2 = how_many_items(5);
    if let Cow::Owned(_) = ic_2 {
        println!("ic_2 is owned, value: {}", ic_2);
    }
}
